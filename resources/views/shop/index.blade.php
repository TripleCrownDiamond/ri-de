<x-public-layout>
    <div class="relative min-h-screen bg-[#FDFDFC]">
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none">
            <div class="absolute -top-[10%] -right-[5%] h-[500px] w-[500px] rounded-full bg-blue-50/50 blur-[120px]"></div>
                ...
                <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
                    <div class="max-w-3xl">
                        <h1 class="text-5xl font-black tracking-tighter text-gray-900 sm:text-7xl uppercase italic">
                            {{ __('Le') }} <span class="text-blue-600">{{ __('Catalogue') }}</span>
                        </h1>
            ...
            <div class="flex flex-col lg:flex-row gap-16">
                        ...
                        <div class="overflow-hidden rounded-[2.5rem] border border-gray-100 bg-white shadow-2xl shadow-gray-200/50 transition-all hover:shadow-red-900/5">
                            ...
                            <form action="{{ route('shop.index') }}" method="GET" class="p-8 space-y-10">
                                ...
                                <div class="group space-y-4">
                                    <label for="search" class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 group-hover:text-blue-600 transition-colors">{{ __('Recherche') }}</label>
                                    <div class="relative">
                                        <input type="text" name="search" id="search" value="{{ request('search') }}" class="block w-full rounded-2xl border-gray-100 bg-gray-50 px-5 py-4 text-sm font-bold text-gray-900 placeholder-gray-300 focus:border-blue-500 focus:bg-white focus:ring-blue-500/20 transition-all" placeholder="{{ __('Modèle, SKU...') }}">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="space-y-2">
                                        @foreach($categories as $category)
                                            <label class="group flex cursor-pointer items-center justify-between rounded-xl p-2 transition-colors hover:bg-blue-50/50">
                                                <div class="flex items-center">
                                                    <div class="relative flex h-5 w-5 items-center justify-center">
                                                        <input type="checkbox" name="category[]" value="{{ $category->slug }}" {{ in_array($category->slug, (array)request('category')) ? 'checked' : '' }} class="peer h-5 w-5 rounded-lg border-gray-200 text-blue-600 focus:ring-blue-500/20 transition-all">
                                                ...
                                                <span class="rounded-lg bg-gray-50 px-2 py-1 text-[10px] font-black text-gray-400 group-hover:bg-blue-600 group-hover:text-white transition-all">{{ $category->products_count }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div class="relative flex-1">
                                            ...
                                            <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Min" class="block w-full rounded-2xl border-gray-100 bg-gray-50 pl-8 pr-4 py-4 text-sm font-bold focus:border-blue-500 focus:bg-white focus:ring-blue-500/20 transition-all">
                                        </div>
                                        <div class="relative flex-1">
                                            <span class="absolute left-4 top-4 text-[10px] font-bold text-gray-400">€</span>
                                            <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="Max" class="block w-full rounded-2xl border-gray-100 bg-gray-50 pl-8 pr-4 py-4 text-sm font-bold focus:border-blue-500 focus:bg-white focus:ring-blue-500/20 transition-all">
                                        </div>
                                    </div>
                                </div>

                                <div class="pt-6 space-y-4">
                                    <button type="submit" class="group relative flex w-full items-center justify-center gap-3 overflow-hidden rounded-2xl bg-gray-900 px-6 py-5 text-xs font-black uppercase tracking-widest text-white shadow-xl transition-all hover:bg-blue-600 hover:shadow-red-900/20 active:scale-95">
                                    ...
                                    @if(request()->anyFilled(['search', 'category', 'brand', 'min_price', 'max_price']))
                                        <a href="{{ route('shop.index') }}" class="flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-blue-600 transition-colors">
                ...
                <div class="flex-1">
                    <div class="mb-12 flex flex-col sm:flex-row sm:items-center justify-between gap-6 rounded-[2rem] bg-white px-8 py-6 shadow-sm border border-gray-100 transition-all hover:shadow-md">
                        <div class="flex items-center gap-4">
                            <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 font-black">
                                {{ $products->total() }}
                            </div>
                            <div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <select name="sort" class="rounded-xl border-gray-100 bg-gray-50 text-xs font-black text-gray-900 focus:border-blue-500 focus:ring-blue-500/20 cursor-pointer py-2.5 px-4 transition-all"
                            ...
                            <div class="group relative flex flex-col overflow-hidden rounded-[2.5rem] border border-gray-100 bg-white p-2 shadow-sm transition-all duration-500 hover:-translate-y-2 hover:border-blue-100 hover:shadow-2xl hover:shadow-red-900/5">
                                ...
                                @if($loop->index < 3)
                                    <div class="absolute top-6 left-6 z-10">
                                        <span class="inline-flex items-center rounded-full bg-blue-600 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-red-600/20">
                                            {{ __('Nouveau') }}
                                        </span>
                                    </div>
                                @endif
                                ...
                                <div class="relative aspect-[4/5] overflow-hidden rounded-[2rem] bg-gray-50">
                                    ...
                                    <div class="absolute inset-x-0 bottom-0 flex translate-y-full items-center justify-center gap-3 p-8 transition-transform duration-500 group-hover:translate-y-0">
                                        <a href="{{ route('products.show', $product) }}" class="flex h-14 w-14 items-center justify-center rounded-2xl bg-white text-gray-900 shadow-2xl transition hover:bg-blue-600 hover:text-white hover:scale-110 active:scale-95">
                                ...
                                <div class="flex flex-1 flex-col p-6 pt-5" x-data="{ qty: 1 }">
                                    <div class="mb-6">
                                        <div class="flex items-center justify-between mb-2">
                                            <span class="text-[10px] font-black uppercase tracking-[0.2em] text-blue-600">{{ $product->brand?->name ?? __('Premium') }}</span>
                                            <span class="text-[10px] font-bold text-gray-400">SKU: {{ $product->sku }}</span>
                                        </div>
                                        <h3 class="line-clamp-2 text-base font-bold tracking-tight text-gray-900 transition-colors group-hover:text-blue-600">
                                    ...
                                    <div class="mb-8 flex items-end justify-between">
                                        <div class="flex flex-col">
                                            ...
                                            @else
                                                <span class="text-sm font-black text-blue-600 uppercase italic">{{ __('Sur devis') }}</span>
                                            @endif
                                        </div>
                                        @if($product->price_ttc)
                                    ...
                                    <div class="mt-auto">
                                        <div class="flex flex-col gap-3">
                                            ...
                                            <a
                                                ...
                                                class="group/btn relative flex w-full items-center justify-center gap-2 overflow-hidden rounded-xl bg-blue-600 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-white shadow-lg shadow-red-900/20 transition-all hover:bg-blue-700 hover:shadow-red-900/40 active:scale-95"
                                            >
