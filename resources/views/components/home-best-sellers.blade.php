    ...
    <div class="mb-12 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <span class="text-xs font-black uppercase tracking-widest text-red-600">{{ __('Populaire') }}</span>
            </div>
            <h2 class="text-4xl font-black tracking-tight text-gray-900 sm:text-5xl">
                {!! __('Nos meilleures <span class="text-red-600">ventes</span> .') !!}
            </h2>
        </div>
        <a href="{{ route('shop.index') }}" class="group inline-flex items-center gap-3 rounded-full bg-gray-900 px-8 py-4 text-xs font-black uppercase tracking-widest text-white shadow-xl shadow-gray-200 transition-all hover:bg-red-600 hover:shadow-red-100 active:scale-95">
        ...
        @foreach ($products as $product)
            <div class="group relative flex flex-col overflow-hidden rounded-[2.5rem] border border-gray-100 bg-white p-2 shadow-sm transition-all duration-500 hover:-translate-y-2 hover:border-red-100 hover:shadow-2xl hover:shadow-red-900/5">
                ...
                <div class="flex flex-1 flex-col p-6 pt-5" x-data="{ qty: 1 }">
                    <div class="mb-4">
                        <span class="text-[10px] font-bold uppercase tracking-widest text-red-600">{{ $product->brand?->name ?? __('Premium') }}</span>
